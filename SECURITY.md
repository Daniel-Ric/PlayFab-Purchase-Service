...
- Bypassing authentication or access controls (e.g., calling protected routes without a valid JWT or required service tokens)
...
This service integrates with external systems (Xbox Live / PlayFab / Minecraft Services / internal marketplace backends). These integrations rely on credentials (API keys, tokens, etc.). Protecting those secrets is critical.
...
- **Least privilege.**  
  Any external service credentials used by PlayFab-Purchase-Service should have only the minimal required permissions, not blanket admin rights.
...
Thank you for helping keep PlayFab-Purchase-Service safe.
